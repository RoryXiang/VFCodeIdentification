...
IMAGE_HEIGHT = 60
IMAGE_WIDTH = 200
MAX_CAPTCHA_4 = 4  # len(text)
# print("验证码文本最长字符数", MAX_CAPTCHA, text)  # 验证码最长4字符; 我全部固定为4,可以不固定.
# 如果验证码长度小于4，用'_'补齐
...
def get_next_batch(batch_size=128):
    ...
    batch_y = np.zeros([batch_size, MAX_CAPTCHA_4 * CHAR_SET_LEN])
...
X_4 = tf.placeholder(tf.float32, [None, IMAGE_HEIGHT * IMAGE_WIDTH])
Y_6 = tf.placeholder(tf.float32, [None, MAX_CAPTCHA_4 * CHAR_SET_LEN])
keep_prob_4 = tf.placeholder(tf.float32)  # dropout


# 定义CNN
def crack_captcha_cnn_4(w_alpha=0.01, b_alpha=0.1):
    x = tf.reshape(X_4, shape=[-1, IMAGE_HEIGHT, IMAGE_WIDTH, 1])
    ...
    conv1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                           padding='SAME')
    conv1 = tf.nn.dropout(conv1, keep_prob_4)
    ...
    conv2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                           padding='SAME')
    conv2 = tf.nn.dropout(conv2, keep_prob_4)
    ...
    conv3 = tf.nn.max_pool(conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                           padding='SAME')
    conv3 = tf.nn.dropout(conv3, keep_prob_4)
    ...
    w_d = tf.Variable(w_alpha * tf.random_normal([4 * 13 * 256, 1024]))
    b_d = tf.Variable(b_alpha * tf.random_normal([1024]))
    dense = tf.reshape(conv4, [-1, w_d.get_shape().as_list()[0]])
    dense = tf.nn.relu(tf.add(tf.matmul(dense, w_d), b_d))
    dense = tf.nn.dropout(dense, keep_prob_4)

    w_out = tf.Variable(
        w_alpha * tf.random_normal([1024, MAX_CAPTCHA_4 * CHAR_SET_LEN]))
    b_out = tf.Variable(
        b_alpha * tf.random_normal([MAX_CAPTCHA_4 * CHAR_SET_LEN]))
...
def train_crack_captcha_cnn():
    ...
    predict = tf.reshape(output, [-1, MAX_CAPTCHA_4, CHAR_SET_LEN])
    max_idx_p = tf.argmax(predict, 2)
    max_idx_l = tf.argmax(tf.reshape(Y_6, [-1, MAX_CAPTCHA_4, CHAR_SET_LEN]), 2)
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            _, loss_ = sess.run([optimizer, loss],
                                feed_dict={X_4: batch_x, Y_6: batch_y,
                                           keep_prob_4: 0.4})
            ...
            if step % 50 == 0:
                ...
                acc = sess.run(accuracy,
                               feed_dict={X_4: batch_x_test, Y_6: batch_y_test,
                                          keep_prob_4: 1.})
                print("iter step: {}, accecery is: {}".format(step, acc))
                # 如果准确率大于98%,保存模型,完成训练
                if acc > 0.98:
                    saver.save(sess, "./src/modle_4_cn4/crack_capcha.model",
                               global_step=step)
                    break
